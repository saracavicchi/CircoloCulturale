...
public class CalendarioCorsoService {
    ...
    public List<CalendarioCorso> findCorsiSovrapposti(Weekday giorno, LocalTime inizio, LocalTime fine, Integer idSala){
        return calendarioCorsoRepository.findCorsiSovrapposti(giorno, inizio, fine, idSala);
    }

    @Transactional //solo calendari active
    public List<CalendarioCorso> findByCorsoId( Integer corsoId){
        return calendarioCorsoRepository.findByCorsoId(corsoId);
    }

    @Transactional //solo calendari active
    public Optional<CalendarioCorso> findByCorsoAndGiornoSettimanaId(Integer corsoId, Weekday giorno){
        return calendarioCorsoRepository.findByCorsoAndGiornoSettimanaId(corsoId, giorno);
    }

    @Transactional //solo calendari active
    public List<CalendarioCorso> findCorsiContemporanei(Weekday giorno, LocalTime inizio, LocalTime fine){
        return calendarioCorsoRepository.findCorsiContemporanei(giorno, inizio, fine);
    }

    @Transactional //solo calendari e corsi active
    public Optional<CalendarioCorso> findSeCorsoContemporaneo(Weekday giorno, Integer idCorso, LocalTime inizio, LocalTime fine){
